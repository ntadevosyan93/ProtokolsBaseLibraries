...
-- =============================================
CREATE PROCEDURE [dbo].[GetBillingsByAccess]
	@type int,
...
BEGIN	
	...
	declare @sql nvarchar(max) --COUNT(id) OVER() AS total,
	...
	if @type =1	begin
		...
		if @type = 2 
			...
			if @type = 3  begin
				...
				if(@c_link>0) begin
					set @sql+= ' and (idm_curator in (select idm_curator_empl from RM$OS$BILLING_EMPL_TO_EMPL with (nolock) where idm_link_empl ='+cast(@idm_user as varchar(10))+' and del=0)'
				end
				else  set @sql+= ' and (idm_curator =(select id from KAD$EMPLOYEE with (nolock)  where idm_controller='+@idm_user+')'
...
EXEC sp_executesql @sql,N'@klass_name varchar(50),@user_cur varchar(50) ,@date_in datetime, @date_to datetime,@StartRowIndex int,
	...
	,@f_descr varchar(250), @f_summ float, @f_depart int, @f_date_plan datetime,@status varchar(250)', @klass_name='billing',@user_cur=@user
	...
	,@f_descr =@f_descr, @f_summ =@f_summ, @f_depart =@f_depart, @f_date_plan =@f_date_plan,@status=@status
 print @sql
end try
begin catch
