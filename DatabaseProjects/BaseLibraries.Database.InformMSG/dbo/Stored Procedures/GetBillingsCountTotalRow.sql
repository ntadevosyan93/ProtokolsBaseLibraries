...
-- =============================================
CREATE PROCEDURE [dbo].[GetBillingsCountTotalRow]
@type int,
...
BEGIN
 SET NOCOUNT ON
	...
	select @idm_user = cast(id as varchar(50)),@idm_control=idm_controller from KAD$EMPLOYEE with (nolock)  where UPPER(KAD$EMPLOYEE.login_ad)=UPPER(@user)
	...
	set @sql ='SELECT COUNT(id) OVER() AS total from dbo.View$RM$OS$BILLING a where del=0 '

	if(@AllowDate =0) set @sql+=' and  cast(date_bill as date) between cast(@date_in as date) and cast(@date_to as date) '

	if @type =1	begin
			if(@countViewOwner>0)  set @sql+=' and idm_owner in(select idm_owner from  [RM$OS$BILLING_StatusView]  with (nolock) where idm_empl='+@idm_user+' and idm_owner is not null) '
		else if(@countWithoutOwner >0)  set @sql +=' and idm_owner not in(select idm_WithoutOwner from  [RM$OS$BILLING_StatusView] with (nolock)  where idm_empl='+@idm_user+' and idm_WithoutOwner is not null) '
	...
	set @sql+= ' ORDER BY [date_bill] DESC'
DECLARE @Error nvarchar(MAX); 

begin try 
EXEC sp_executesql @sql,N'@klass_name varchar(50),@user_cur varchar(50) ,@date_in datetime, @date_to datetime,@f_number varchar(150),@f_date datetime,@f_partner int,@f_owner int, @f_curator varchar(150)
...
,@date_in =@date_in, @date_to = @date_to,@f_number =@f_number,@f_date =@f_date,@f_partner =@f_partner,@f_owner =@f_owner, @f_curator =@f_curator
