...
-- =============================================
CREATE PROCEDURE [dbo].[NeedSendCurator]
...
BEGIN
	...
	select @id=id, @curator_sended=curator_sended, @curator_id = idm_empl from DOGOVORS$REG  with (nolock) where guid = @dogovor_guid
	...
	if (@result_send=1) AND (@curator_sended=0) AND not(@curator_id is null) begin
		update DOGOVORS$REG set curator_sended = @result_send where id = @id
		select @curator_id	
	end else begin
		update DOGOVORS$REG set curator_sended = @result_send where id = @id
		select 'error'	
