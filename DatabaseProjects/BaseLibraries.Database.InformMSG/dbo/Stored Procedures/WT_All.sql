...
from  KAD$WORK_TIME_CURATORS with (nolock)
left join View$KAD$EMPLOYEE with (nolock) on View$KAD$EMPLOYEE.id=idm_curator
 where idm_empl = [KAD$WORK_TIME].idm_empl  
FOR XML PATH('')) as curatorsText
  FROM [InformMSG].[dbo].[KAD$WORK_TIME] with (nolock)
