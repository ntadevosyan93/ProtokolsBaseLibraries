CREATE VIEW dbo.View$SSMP$TEAM_ACTIVE
AS
SELECT        dbo.SSMP$TEAM.id, dbo.SSMP$EMPLOYEE.fio, dbo.AVTO.text, dbo.AVTO.phone AS phone_avto, dbo.SSMP$EMPLOYEE.phone AS phone_doc, dbo.AVTO.mesto, 
                         dbo.AVTO.gos_nomer + ', ' + dbo.AVTO.mesto + ' (' + dbo.SSMP$EMPLOYEE.fio + ')' AS full_name, dbo.SSMP$EMPLOYEE.pediatr,
                             (SELECT        COUNT(id) AS Expr1
                               FROM            dbo.SSMP$MAIN
                               WHERE        (status = 2) AND (del = 0) AND (NOT (guid IS NULL)) AND (idm_team = dbo.SSMP$TEAM.id)) AS orderCount, dbo.AVTO.gos_nomer + ', ' + dbo.AVTO.mesto AS ftitle
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'View$SSMP$TEAM_ACTIVE';
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'View$SSMP$TEAM_ACTIVE';
